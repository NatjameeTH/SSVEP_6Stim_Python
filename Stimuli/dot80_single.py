...
def run():
        ...
        if step_index < len(sequence):
            ...
            if step["type"] == "fixation":
                ...
                if elapsed < 0.05:  # 50 ms แรก
                    print(f"Stimulus {step['freq']} Hz started at {time.strftime('%H:%M:%S')}")
                    for stim in stimuli:
            ...
            if current_time - step_start >= step["duration"]:
                if step["type"] == "stim":
                    ...
                    print(f"Stimulus {step['freq']} Hz flickered for {minutes:.2f} minutes")
                step_index += 1
    ...
    pygame.quit()
    print("Stimulus order: Dot80Sing", stim_log)
    # save_run_log("Dot80Single", "P01", stim_log, trial_records)
